...
LANE_FACTOR_WEIGHT = 1.0
LANE_FACTOR_EASING = 'linear'
DISTANCE_FACTOR_WEIGHT = 0.0
DISTANCE_FACTOR_EASING = 'linear'
...
def reset_globals(params):
  ...
  g_last_progress_value = 0.0
  g_last_progress_time = time.time() 
  g_last_speed_value = params['speed']
  g_last_steering_angle = params['steering_angle']
...
def reward_function(params):
  # Reset state at beginning of an episode
  if steps == 0:
    reset_globals(params)

  """Reward function is:

  f(s,w,h,t,p,d) = 1.0 * W(s,Ks) * W(w,Kw) * W(h,Kh) * W(t,Kt) * W(p,Kp) * W(l,Kl) * W(d,Kd)
  ...
  # l: Lane Factor
  distance_factor = calculate_distance_factor(params)
  ...
  reward *= apply_weight(distance_factor, LANE_FACTOR_WEIGHT, LANE_FACTOR_EASING)
...
def heading_relative_to_target(waypoints, closest_waypoints, car_heading):
  closest_waypoint1 = waypoints[closest_waypoints[0]]
  closest_waypoint2 = waypoints[closest_waypoints[1]]
  track_heading = angle_of_vector((closest_waypoint1, closest_waypoint2))
  print("Track heading: %0.2f" % track_heading)
  print("Car heading: %0.2f" % car_heading)
  # Compare heading to track direction
  delta_angle = abs(track_heading - car_heading)
  print("Delta angle (unclamped): %0.2f" % delta_angle)
  if delta_angle > 180:
      delta_angle = 360 - delta_angle
  print("Delta angle (clamped): %0.2f" % delta_angle)

  target_angle_delta = HEADING_DELTA_TARGET
  print("Target angle delta: %0.2f" % target_angle_delta)

  # 0 degrees off:  1.0 - abs(60 - 60) / 30 = 1 - 0/30 = 1.0
  # 15 degrees off: 1.0 - abs(60 - 45) / 30 = 1 - 15/30 = 0.5
  # 20 degrees off: 1.0 - abs(60 - 40) / 30 = 1 - 20/30 = 0.33
  # 30 degrees off: 1.0 - abs(60 - 40) / 30 = 1 - 20/30 = 0.33
  reward = max(1.0 - (abs(target_angle_delta - delta_angle) / HEADING_RANGE),
               0.0)
...
def distance_traveled_over_ground(x,y):
  return distance_between_points(g_last_position, (x,y))
  

def calculate_distance_factor(params):
  """ Calulcate the reward for the distance traveled
  """
  lane_factor = distance_traveled_over_ground(params['track_width'],
                                                      params['distance_from_center'])
  return min(lane_factor, 1.0)
