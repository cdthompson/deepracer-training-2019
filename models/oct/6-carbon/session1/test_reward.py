...
class TestReward(unittest.TestCase):
  ...
  def test_percentage_steering_angle(self):
    ...
    self.assertTrue(math.isclose(reward.percentage_steering_angle(15.0), 0.5))
    self.assertTrue(math.isclose(reward.percentage_steering_angle(20.0), 1/3))
    self.assertTrue(math.isclose(reward.percentage_steering_angle(30.0), 0.0))
    self.assertTrue(math.isclose(reward.percentage_steering_angle(-30.0), 0.0))
    self.assertTrue(math.isclose(reward.percentage_steering_angle(-20.0), 1/3))
  ...
  def test_race_line_heading(self):
    ...
    reward.g_race_line_dists = [LineString(reward.RACE_LINE_WAYPOINTS).project(Point(p), normalized=True) for p in LineString(reward.RACE_LINE_WAYPOINTS).coords[:-1]] + [1.0]
    ...
    params['heading'] = 0
    self.assertTrue(math.isclose(reward.race_line_heading(params), 1.0, abs_tol=1e-2))
    params['heading'] = 45
    self.assertTrue(math.isclose(reward.race_line_heading(params), 0.5, abs_tol=1e-2))
    params['heading'] = -45
    self.assertTrue(math.isclose(reward.race_line_heading(params), 0.5, abs_tol=1e-2))
    params['heading'] = 90
    self.assertTrue(math.isclose(reward.race_line_heading(params), 0.0, abs_tol=1e-2))
    params['heading'] = -90
    self.assertTrue(math.isclose(reward.race_line_heading(params), 0.0, abs_tol=1e-2))
    params['heading'] = 30
    self.assertTrue(math.isclose(reward.race_line_heading(params), 0.66, abs_tol=1e-2))

    # close to -170
    params['x'] = reward.OVAL_TRACK_RACE_LINE[65][0] 
    params['y'] = reward.OVAL_TRACK_RACE_LINE[65][1]
    params['heading'] = 170
    self.assertTrue(math.isclose(reward.race_line_heading(params), 0.81, abs_tol=1e-2))
    # close to 170
    params['x'] = reward.OVAL_TRACK_RACE_LINE[55][0] 
    params['y'] = reward.OVAL_TRACK_RACE_LINE[55][1]
    params['heading'] = -170
    self.assertTrue(math.isclose(reward.race_line_heading(params), 0.84, abs_tol=1e-2))
    ...
    self.assertTrue(math.isclose(reward.race_line_heading(params), 0.94, abs_tol=1e-2))
  ...
  def test_progress_target(self):
    ...
    reward.g_last_progress_value = 0.0
    self.assertEqual(reward.progress_target(1.0), 1.0)
    reward.g_last_progress_value = 0.0
    self.assertEqual(reward.progress_target(0.5), 0.5)
    reward.g_last_progress_value = 10.0
    self.assertEqual(reward.progress_target(10.5), 0.5)
    reward.TARGET_NUMBER_STEPS = 300
    reward.g_last_progress_value = 0.0
    self.assertTrue(math.isclose(reward.progress_target(0.333), 1.0, abs_tol=1e-2))
    reward.g_last_progress_value = 0.0
    self.assertTrue(math.isclose(reward.progress_target(0.2), 0.6))
    reward.g_last_progress_value = 10.0
    self.assertTrue(math.isclose(reward.progress_target(10.2), 0.6))
  ...
  def test_calculate_race_line_factor(self):
    ...
    params['x'] = reward.OVAL_TRACK_RACE_LINE[9][0] 
    params['y'] = reward.OVAL_TRACK_RACE_LINE[9][1] - params['track_width'] / 4.0
    ...
    params['x'] = reward.OVAL_TRACK_RACE_LINE[35][0] + params['track_width'] / 4.0 
    params['y'] = reward.OVAL_TRACK_RACE_LINE[35][1]
    self.assertTrue(math.isclose(reward.calculate_race_line_factor(params), 0.5, abs_tol=1e-2))
    ...
    params['x'] = reward.OVAL_TRACK_RACE_LINE[28][0] + (params['track_width'] / math.sqrt(2)) / 4.0 
    params['y'] = reward.OVAL_TRACK_RACE_LINE[28][1] - (params['track_width'] / math.sqrt(2)) / 4.0
    self.assertTrue(math.isclose(reward.calculate_race_line_factor(params), 0.5, abs_tol=1e-2))
