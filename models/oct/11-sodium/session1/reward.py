...
def checkpoint_for_progress(progress):
    for i in range(len(PROGRESS_CHECKPOINTS)):
        if PROGRESS_CHECKPOINTS[i] > progress:
            return i-1
    return -1
...
def reward_function(params):
    ...
    if params['steps'] == 0:
        ...
        if previous_checkpoint_index < current_checkpoint_index:
            # passed a checkpoint
            budget_checkpoint_steps = (TARGET_STEPS / 100.0) * (PROGRESS_CHECKPOINTS[current_checkpoint_index] - PROGRESS_CHECKPOINTS[previous_checkpoint_index])
