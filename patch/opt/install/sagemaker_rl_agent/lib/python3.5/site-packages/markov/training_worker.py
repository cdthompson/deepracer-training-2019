...
def main():
  ...
  try:
    ...
    parser.add_argument('--redis_host',
                        help='(string) Redis host',
                        type=str,
                        default=os.environ.get("REDIS_HOST", socket.gethostname()))

    args, unknown = parser.parse_known_args()

    s3_client = SageS3Client(bucket=args.s3_bucket, s3_prefix=args.s3_prefix, aws_region=args.aws_region, endpoint_url=args.aws_endpoint_url)
    ...
    host_ip_address = get_ip_from_host(args.redis_host)
    ...
    if use_pretrained_model:
        logger.info("Restoring pretrained model at bucket:%s prefix:%s" % (args.pretrained_s3_bucket, args.pretrained_s3_prefix))
    ...
    memory_backend_params = RedisPubSubMemoryBackendParameters(redis_address=args.redis_host,
                                                               redis_port=6379,
                                                               run_type='trainer',
