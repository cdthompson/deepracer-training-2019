...
# Workaround for coach bug https://github.com/NervanaSystems/coach/issues/404
from rl_coach import logger as clogger
clogger.experiment_path = "/opt/ml/model"
...
class DeepRacerRacetrackEnv(gym.Env):
    ...
    def __init__(self):
        ...
        if node_type == SIMULATION_WORKER:
            ...
            try:
                ...
                # ---
                # bool success
                resp = utils.gazebo_service_call(set_physics_properties, update_msg)
                
            except Exception as ex:
                utils.json_format_logger("Unable to set physics update rate: {}".format(ex),
    ...
    def setup_simtrace_data_upload_to_s3(self):
        if node_type == SIMULATION_WORKER:
            ...
            if self.job_type == TRAINING_JOB:
                ...
                self.simtrace_s3_endpoint_url = rospy.get_param('AWS_ENDPOINT_URL')
            ...
            self.simtrace_data = DeepRacerRacetrackSimTraceData(self.simtrace_s3_bucket, self.simtrace_s3_key, self.simtrace_s3_endpoint_url)
    ...
    def infer_reward_state(self, steering_angle, speed):
        ...
        logger.info('SIM_TRACE_LOG:%d,%d,%.4f,%.4f,%.4f,%.2f,%.2f,%d,%.4f,%s,%s,%.4f,%d,%.2f,%s,%.4f\n' % (
    ...
    def write_metrics_to_s3(self):
        session = boto3.session.Session()
        s3_client = session.client('s3', region_name=self.aws_region, endpoint_url=self.aws_endpoint_url)
    ...
    def cancel_simulation_job(self):
        ...
        # CT: TODO: is there any other housekeeping we need to do here to mark an evaluation done?
        #session = boto3.session.Session()
        #robomaker_client = session.client('robomaker', region_name=self.aws_region, endpoint_url=self.aws_endpoint_url)
