...
class S3BotoDataStoreParameters(DataStoreParameters):
    def __init__(self, aws_region: str = "us-west-2", bucket_name: str = None, s3_folder: str = None,
            checkpoint_dir: str = None, aws_endpoint_url: str = None):
        super().__init__("s3", "", "")
...
class S3BotoDataStore(DataStore):
    ...
    def _get_client(self):
        ...
        return session.client('s3', region_name=self.params.aws_region, endpoint_url=self.params.aws_endpoint_url)
    ...
    def load_from_store(self, expected_checkpoint_number=-1):
        try:
            ...
            if len(os.listdir(self.params.checkpoint_dir)) > 0:
                ...
                utils.json_format_logger('Removing %d old checkpoint files' % len(files_to_remove))
                for f in files_to_remove:
                    try:
                        os.unlink(os.path.abspath(os.path.join(self.params.checkpoint_dir, f)))
                    except Exception as e:
                        # swallow errors
                        pass
            ...
            while True:
                ...
                if "Contents" not in response:
                    try:
                        ...
                        utils.json_format_logger("Sleeping {} seconds while lock file is present"
                                                .format(SLEEP_TIME_WHILE_WAITING_FOR_DATA_FROM_TRAINER_IN_SECOND))
